...
class NameThePlayerGame:
    """Manages a 'Name the Player' game where players identify soccer players."""
    ...
    def __init__(self, rounds_limit: int = 15, used_images: Optional[List[str]] = None):
        ...
        self.players_data: List[Tuple[str, str]] = [] # list of (path, player_name)
    ...
    def _load_data(self):
        """Load data from the soccer-players directory."""
        image_dir = os.path.join(os.path.dirname(__file__), "soccer-players")
        
        try:
            if os.path.exists(image_dir):
                for filename in os.listdir(image_dir):
                    if filename.lower().endswith(('.webp', '.png', '.jpg', '.jpeg')):
                        # Remove extension to get the player name
                        clean_name = os.path.splitext(filename)[0]
                        self.players_data.append((os.path.join(image_dir, filename), clean_name))
        except Exception as e:
            print(f"Error loading Name the Player data: {e}")
    ...
    def start_new_round(self) -> Optional[Tuple[str, int]]:
        ...
        available_images = [d for d in self.players_data if d[0] not in self.used_images]
        if not available_images:
            # If all images have been used, loop back
            self.used_images.clear()
            available_images = self.players_data
